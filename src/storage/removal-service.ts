...
export class RemovalService {
  ...
  private readonly operations = new Map<string, RemovalOperation>()
  ...
  async prepare(name: string): Promise<PreparedRemovalDocument> {
    this.pruneExpired()
    ...
    try {
      ...
      if (inspection.state === 'invalid' && inspection.currentHash === undefined) {
        throw new RemovalError(
          'remove-failed',
          'This invalid skill cannot be fingerprinted safely and must not be removed automatically.',
        )
      }
  ...
  confirm(operationId: string): Promise<string> {
    ...
    if (operation === undefined || operation.expiresAt <= this.now().getTime()) {
      return Promise.reject(new RemovalError(
        'operation-expired',
        'The prepared removal expired. Prepare it again.',
      ))
    }
  ...
  private async commitRemoval(operation: RemovalOperation): Promise<string> {
    ...
    if (currentRecord === undefined || !managedRecordsEqual(currentRecord, operation.record)) {
      throw new RemovalError(
        'state-changed',
        'The Managed Skill changed after removal was prepared.',
      )
    }
    ...
    try {
      ...
      const remainingSkills = Object.fromEntries(
        Object.entries(manifest.skills).filter(([name]) => name !== currentRecord.name),
      )
      ...
      try {
        ...
        throw new RemovalError(
          'rollback-failed',
          'Removal failed and automatic rollback did not complete.',
    ...
    if (committed && moved) {
      try {
        ...
        this.warn(
          'The skill was removed, but its in-root recovery backup could not be deleted.',
          cleanupError,
        )
      }
    }

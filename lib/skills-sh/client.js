...
export class SkillsShClient {
    ...
    async download(id, signal) {
        ...
        if (this.downloadBlockedUntil > this.now()) {
            const retryAfterSeconds = Math.max(1, Math.ceil((this.downloadBlockedUntil - this.now()) / 1000));
            throw rateLimitError(retryAfterSeconds);
        }
        const encodedId = catalogId.split('/').map(encodeURIComponent).join('/');
        try {
            const response = await this.requestJson(new URL(`/api/download/${encodedId}`, this.baseUrl), signal, SNAPSHOT_RESPONSE_LIMIT_BYTES);
    ...
    cacheSnapshot(id, snapshot) {
        ...
        if (bytes > this.snapshotCacheMaxBytes)
            return;
...
function snapshotSize(snapshot) {
    return Buffer.byteLength(snapshot.hash, 'utf8')
        + snapshot.files.reduce((total, file) => total
            + Buffer.byteLength(file.path, 'utf8')
            + Buffer.byteLength(file.contents, 'utf8'), 0);
}
...
function rateLimitError(retryAfterSeconds) {
    ...
    return new SkillsShError('rate-limited', `Skills.sh request limit reached. Try again in about ${minutes} minute${minutes === 1 ? '' : 's'}.`, 429, retryAfterSeconds);
}
